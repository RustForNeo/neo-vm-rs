...
use crate::compound_type::CompoundType;
...
use crate::primitive_type::{PrimitiveType, PrimitiveTypeTrait};
use crate::reference_counter::ReferenceCounter;
use crate::slot::Slot;
use crate::stack_item::StackItem;
use crate::stack_item::StackItem::{VMArray, VMInteger};
use crate::stack_item_type::StackItemType;
use crate::stack_item_type::StackItemType::Array;
use crate::Struct::Struct;
use crate::vm::script::Script;
use crate::vm::vm_exception::{Error, VMException};
...
pub struct ExecutionEngine<'a> {
    ...
    pub reference_counter: ReferenceCounter<'a>,

    /// The invocation stack of the VM.
    pub invocation_stack: Vec<ExecutionContext<'a>>,

    /// The top frame of the invocation stack.
    pub current_context: Option<ExecutionContext<'a>>,

    /// The bottom frame of the invocation stack.
    pub entry_context: Option<ExecutionContext<'a>>,

    /// The stack to store the return values.
    pub result_stack: EvaluationStack<'a>,
...
impl ExecutionEngine {
    ...
    pub fn with_options(limits: ExecutionEngineLimits) -> Self {
        Self {
            ...
            entry_context: None,
            result_stack: EvaluationStack::new(&ReferenceCounter::new()),
    ...
    fn execute_instr(&mut self, instr: Instruction) -> Result<VMState, VMException> {
        match instr.opcode {
            ...
            OpCode::PushA =>
                {
                    let position = (self.current_context.unwrap().instr_pointer as i32).checked_add(instr.token_i32()).unwrap();
                    if position < 0 || position > self.current_context.unwrap().shared_states.script.len() as i32 {
                        // return Err(VMException::InvalidOpcode("Bad pointer address: {position}");
                        Err(VMException::new(Error::new("Bad pointer address")))
                    }

                    self.push(StackItem::VMPointer(Pointer::new(self.current_context.unwrap().shared_states.script, position as usize)))
                }
            ...
            OpCode::Call => self.execute_call((self.current_context.unwrap().instruction_pointer + instr.token_i8()) as i32),
            OpCode::CallL => self.execute_call(self.current_context.unwrap().InstructionPointer + instr.token_i32()),
            OpCode::CallA =>
                {
                    let x: Pointer = self.pop().into();
                    if x.Script != self.current_context.unwrap().Script {
                        return Err(VMException::InvalidOpcode("Pointers can't be shared between scripts".parse().unwrap()));
                    }
                    self.execute_call(x.Position)
                }
            OpCode::CallT => self.load_token(instr.token_u16()),
            OpCode::Abort => Err(VMException::InvalidOpcode("{OpCode::ABORT} is executed.".parse().unwrap())),
            OpCode::Assert =>
                {
                    let x = self.pop().get_bool();
                    if !x {
                        Err(VMException::InvalidOpcode("{OpCode::ASSERT} is executed with false result.".parse().unwrap()))
                    }
                    // break;
                }
            ...
            OpCode::EndFinally =>
                {
                    if self.current_context.unwrap().try_stack.is_none() {
                        return Err(VMException::InvalidOpcode("The corresponding TRY block cannot be found.".parse().unwrap()));
                    }
                    let current_try = match self.current_context.unwrap().try_stack {
                        Some(ref mut x) => x,
                        None => return Err(VMException::InvalidOpcode("The corresponding TRY block cannot be found.".parse().unwrap())),
                    };
            ...
            OpCode::Ret =>
                {
                    ...
                    let stack_eval: EvaluationStack = match self.invocation_stack.len() == 0 {
                        true => self.result_stack.clone(),
                        false => self.invocation_stack.last().unwrap().borrow().evaluation_stack.clone(),
                    };
                    // }
                    ...
                    if context_pop.EvaluationStack != stack_eval
                    {
                        if context_pop.RVCount >= 0 && context_pop.EvaluationStack.Count != context_pop.RVCount {
                            return Err(VMException::InvalidOpcode("RVCount doesn't match with EvaluationStack".parse().unwrap()));
                        }
            ...
            OpCode::Xdrop =>
                {
                    ...
                    if n < 0 {
                        return Err(VMException::InvalidOpcode("The negative value {n} is invalid for OpCode::{instr.OpCode}.".parse().unwrap()));
                    }
            ...
            OpCode::Tuck => self.current_context.unwrap().shared_states.evaluation_stack.Insert(2, self.peek(0)),
            OpCode::Swap =>
                {
            ...
            OpCode::InitSSLot =>
                {
                    if self.current_context.unwrap().shared_states.static_fields.is_some() {
                        return Err(VMException::InvalidOpcode("{instr.OpCode} cannot be executed twice.".parse().unwrap()));
                    }
                    if instr.token_u8() == 0 {
                        return Err(VMException::InvalidOpcode("The operand {instr.token_u8()} is invalid for OpCode::{instr.OpCode}.".parse().unwrap()));
                    }
                    self.current_context.unwrap().shared_states.static_fields = Some(Slot::new_with_count(instr.token_u8() as i32, &self.reference_counter))
                    // break;
                }
            OpCode::InitSlot =>
                {
                    if self.current_context.unwrap().local_variables.is_some() || self.current_context.unwrap().arguments.is_some() {
                        return Err(VMException::InvalidOpcode("{instr.OpCode} cannot be executed twice.".parse().unwrap()));
                    }
                    if instr.token_u16() == 0 {
                        return Err(VMException::InvalidOpcode("The operand {instr.token_u16()} is invalid for OpCode::{instr.OpCode}.".parse().unwrap()));
                    }
                    if instr.token_u8() > 0
                    {
                        self.current_context.unwrap().local_variables =Some(Slot::new_with_count(instr.token_u8() as i32, &self.reference_counter));
                    }
                    if instr.token_u8_1() > 0
                    {
                        ...
                        self.current_context.unwrap().arguments = Some(Slot::new(items, &self.reference_counter))
                    }
                }
            OpCode::LdSFLd0 |
            OpCode::LdSFLd1 |
            ...
            OpCode::LdSFLd4 |
            OpCode::LdSFLd5 |
            OpCode::LdSFLd6 => self.execute_load_from_slot(&mut self.current_context.unwrap().shared_states.static_fields.unwrap(), instr.OpCode - OpCode::LdSFLd0),
            OpCode::LdSFLd => self.execute_load_from_slot(&mut self.current_context.unwrap().shared_states.static_fields.unwrap(), instr.token_u8() as usize),
            OpCode::StSFLd0 |
            OpCode::StSFLd1 |
            OpCode::StSFLd2 |
            OpCode::StSFLd3 |
            OpCode::StSFLd4 |
            OpCode::StSFLd5 |
            OpCode::StSFLd6 => self.execute_store_to_slot(&mut self.current_context.unwrap().shared_states.static_fields, instr.OpCode - OpCode::StSFLd0),
            OpCode::StSFLd => self.execute_store_to_slot(&mut self.current_context.unwrap().shared_states.static_fields, instr.token_u8() as usize),
            OpCode::LdLoc0 |
            OpCode::LdLoc1 |
            OpCode::LdLoc2 |
            OpCode::LdLoc3 |
            OpCode::LdLoc4 |
            OpCode::LdLoc5 |
            OpCode::LdLoc6 => self.execute_load_from_slot(self.current_context.unwrap().shared_states.local_variables, instr.OpCode - OpCode::LdLoc0),
            OpCode::LdLoc => self.execute_load_from_slot(self.current_context.unwrap().shared_states.local_variables, instr.token_u8() as usize),
            OpCode::StLoc0 |
            OpCode::StLoc1 |
            OpCode::StLoc2 |
            OpCode::StLoc3 |
            OpCode::StLoc4 |
            OpCode::StLoc5 |
            OpCode::StLoc6 => self.execute_store_to_slot(self.current_context.unwrap().shared_states.local_variables, instr.OpCode - OpCode::StLoc0),
            OpCode::StLoc => self.execute_store_to_slot(self.current_context.unwrap().shared_states.local_variables, instr.token_u8() as usize),
            OpCode::LdArg0 |
            OpCode::LdArg1 |
            OpCode::LdArg2 |
            OpCode::LdArg3 |
            OpCode::LdArg4 |
            OpCode::LdArg5 |
            OpCode::LdArg6 => self.execute_load_from_slot(&mut self.current_context.unwrap().arguments.unwrap(), instr.OpCode - OpCode::LdArg0),
            OpCode::LdArg => self.execute_load_from_slot(&mut self.current_context.unwrap().arguments.unwrap(), instr.token_u8() as usize),
            OpCode::StArg0 |
            OpCode::StArg1 |
            OpCode::StArg2 |
            OpCode::StArg3 |
            OpCode::StArg4 |
            OpCode::StArg5 |
            OpCode::StArg6 => self.execute_store_to_slot(&mut self.current_context.unwrap().arguments, instr.OpCode - OpCode::StArg0),
            OpCode::StArg => self.execute_store_to_slot(&mut self.current_context.unwrap().arguments, instr.token_u8() as usize),
            ...
            OpCode::MemCpy =>
                {
                    ...
                    if count < 0 {
                        return Err(VMException::InvalidOpcode("The value {count} is out of range.".parse().unwrap()));
                    }
                    let si = self.pop().get_integer().to_i64().unwrap();
                    if si < 0 {
                        return Err(VMException::InvalidOpcode("The value {si} is out of range.".parse().unwrap()));
                    }
                    ReadOnlySpan < byte > src = self.pop().GetSpan();
                    if si.checked_add(count) > src.Length {
                        return Err(VMException::InvalidOpcode("The value {count} is out of range.".parse().unwrap()));
                    }
                    let di = self.pop().get_integer().to_i64().unwrap();
                    if (di < 0) {
                        return Err(VMException::InvalidOpcode("The value {di} is out of range.".parse().unwrap()));
                    }
            ...
            OpCode::Substr =>
                {
                    ...
                    if index < 0 {
                        return Err(VMException::InvalidOpcode("The value {index} is out of range.".parse().unwrap()));
                    }
            ...
            OpCode::Sqrt =>
                {
                    self.push(self.pop().get_integer().Sqrt())
                }
            OpCode::ModMul =>
                {
            ...
            OpCode::Shl =>
                {
                    ...
                    if shift == 0 { return Ok(VMState::None); }
                    let x = self.pop().get_integer();
            ...
            OpCode::Lt =>
                {
                    let x2 = self.pop();
                    let x1 = self.pop();
                    if x1.get_item_type() == StackItemType::Any || x2.get_item_type() == StackItemType::Any {
                        self.push(StackItem::from(false))
                    } else {
            ...
            OpCode::PackMap =>
                {
                    ...
                    if size < 0 || size * 2 > self.current_context.unwrap().evaluation_stack.Count {
                        return Err(VMException::InvalidOpcode("The value {size} is out of range.".parse().unwrap()));
                    }
                    let map = Map::new(Some(&self.reference_counter));
                    for i in 0..size
                    {
            ...
            OpCode::PackStruct =>
                {
                    ...
                    let _struct = Struct::new(self.reference_counter);
                    for i in 0..size
                    {
                        let item = self.pop();
                        _struct.Add(item);
            ...
            OpCode::Pack =>
                {
                    let size = self.pop().get_integer().to_usize().unwrap();
                    if size < 0 || size > self.current_context.unwrap().shared_states.evaluation_stack.len() {
                        return Err(VMException::InvalidOpcode("The value {size} is out of range.".parse().unwrap()));
                    }
                    let array = Array::new(&self.reference_counter);
                    for i in 0..size
                    {
                        let item = self.pop();
                        array.Add(item);
            ...
            OpCode::Unpack =>
                {
                    ...
                    match compound
                    {
                        ...
                        // break;
                        _ => return Err(VMException::InvalidOpcode("Invalid type for {instr.OpCode}: {compound.Type}".parse().unwrap()))
                    }
                    self.push(StackItem::from(compound.Count))
                }
            OpCode::NewArray0 =>
                self.push(StackItem::VMArray(Array::new(&self.reference_counter))),
            OpCode::NewArray |
            OpCode::NewArrayT =>
                {
                    let n = self.pop().get_integer();
                    if n < BigInt::zero() || n > self.limits.MaxStackSize {
                        return Err(VMException::InvalidOpcode("MaxStackSize exceed: {n}".parse().unwrap()));
                    }
                    let item: StackItem;
                    if instr.OpCode == OpCode::NewArrayT
                    {
                        ...
                        if !StackItemType::is_valid(_type) {
                            return Err(VMException::InvalidOpcode("Invalid type for {instr.OpCode}: {instr.token_u8()}".parse().unwrap()));
                        }
                        item = match _type as StackItemType
                        {
                            ...
                            StackItemType::ByteString => StackItem::VMByteString(ByteString::new(Vec::new())),
                            _ => StackItem::VMNull(Null::default())
                        };
                    } else {
                        item = StackItem::VMNull(Null::default());
                    }
                    self.push(StackItem::from(Array::new(&self.reference_counter, Enumerable.Repeat(item, n))))
                    // break;
                }
            OpCode::NewStruct0 => self.push(StackItem::from(Struct::new(None,Some(&self.reference_counter)))),
            OpCode::NewStruct =>
                {
                    ...
                    let result = Struct::new(None, Some(&self.reference_counter));
                    for  i in 0..n{
            ...
            OpCode::NewMap => self.push(StackItem::from(Map::new(Some(&self.reference_counter)))),
            OpCode::Size =>
                {
                    let x = self.pop();
                    match x
                    {
                        ...
                        _ => return Err(VMException::InvalidOpcode("Invalid type for {instr.OpCode}: {x.Type}".parse().unwrap()))
                    }
                }
            OpCode::HasKey =>
                {
                    ...
                    match x
                    {
                        ...
                        StackItem::VMByteString(array) => {
                            ...
                            if index < 0 {
                                return Err(VMException::InvalidOpcode("The negative value {index} is invalid for OpCode::{instr.OpCode}.".parse().unwrap()));
                            }
            ...
            OpCode::Values =>
                {
                    ...
                    let new_array = Array::new(&self.reference_counter);
                    ...
                    match x
                    {
                        ...
                        StackItem::VMMap(map)=>
                        {
                            let value = match map.get(&key){
                                Some(v) => v,
                                None => return Err(VMException::InvalidOpcode("Key not found in {nameof(Map)}".parse().unwrap())),
                            };
                        ...
                        self.push(StackItem::from((BigInteger)byteArray[index]))
                        }
                        StackItem::VMBuffer(buffer)=>
                        {
                        let index = key.get_integer().to_i64().unwrap();
                        ...
                        self.push(StackItem::from((BigInteger)buffer.InnerBuffer.Span[index]))
                        }
                        _=>
                        return Err(VMException::InvalidOpcode("Invalid type for {instr.OpCode}: {x.Type}".parse().unwrap()))
                    }
                    // break;
                }
            OpCode::Append =>
                {
                    let new_item = self.pop();
                        ...
                        StackItem::VMBuffer(buffer) =>
                            {
                                ...
                                if !StackItemType::is_primitive(value.get_item_type() as u8) {
                                    return Err(VMException::InvalidOpcode("Value must be a primitive type in {instr.OpCode}".parse().unwrap()));
                                }
                                let b = value.get_integer().to_i64().unwrap();
                                if b < i8::min as i64 || b > i8::max as i64 {
                                    return Err(VMException::InvalidOpcode("Overflow in {instr.OpCode}, {b} is not a byte type.".parse().unwrap()));
                                }
                                buffer.InnerBuffer.Span[index] = b
                            }
                        _ =>
                            Err(VMException::InvalidOpcode("Invalid type for {instr.OpCode}: {x.Type}".parse().unwrap()))
                    }
                    // break;
                }
            OpCode::ReverseItems =>
                {
                    ...
                    match x
                    {
                        ...
                        _ => Err(VMException::InvalidOpcode("Invalid type for {instr.OpCode}: {x.Type}".parse().unwrap())),
                    }
                }
            OpCode::Remove =>
                {
                    ...
                    match (x)
                    {
                        ...
                        StackItem::VMMap(map) => map.Remove(key),
                        _ =>
                            return Err(VMException::InvalidOpcode("Invalid type for {instr.OpCode}: {x.Type}".parse().unwrap()));
                    }
                    // break;
            ...
            OpCode::AbortMsg =>
                {
                    ...
                    Err(VMException::InvalidOpcode("{OpCode::ABORTMSG} is executed. Reason: {msg}".parse().unwrap()))
                }
            OpCode::AssertMsg =>
                {
                    let msg = self.pop().GetString();
                    ...
                    if !x
                    {
                        return Err(VMException::InvalidOpcode("{OpCode::ASSERTMSG} is executed with false result. Reason: {msg}".parse().unwrap()));
                    }
                    // break;
                }
    ...
    fn execute_jump_offset(&mut self, offset: i32) {
       self.execute_jump((self.current_context.unwrap().instr_pointer as i32).checked_add(offset).unwrap())
    }
    ...
    fn handle_error(&mut self, err: Error) {
        ...
        self.uncaught_exception = Some(StackItem::Error(err));
    }

    fn load_context(&mut self, context: ExecutionContext) {
        self.invocation_stack.push(context);
        self.current_context = self.invocation_stack.last_mut();
    ...
    fn unload_context(&mut self, mut context: ExecutionContext) {
        ...
        if self.invocation_stack.is_empty() {
            ...
            self.current_context = Some(self.invocation_stack.last_mut().unwrap());
        }
    ...
    fn create_context(&self, script: Script, rvcount: i32, initial_position: usize) -> ExecutionContext {
        ...
        ExecutionContext {
            ...
            rvcount,
            reference_counter: Rc::clone(&self.reference_counter),
    ...
    fn load_script(&mut self, script: Script, rvcount: i32, initial_position: usize) -> Rc<RefCell<ExecutionContext>> {
